!function (a) {
    ...
    var b = 0, c = {}, d = {
        defaults: {
            ...
            input: {
                active: {
                    ...
                    fn: function () { return a(this).attr("name").match(/im\[([\d]+)\]\[([^\)]+)\]/).slice(1) }
                },
                shape: {
            ...
            f.on("mousemove touchmove",
                function (f) {
                    ...
                    var i = "undefined" != typeof f.originalEvent.touches || !1,
                        ...
                        o = [], p = !1; a.each(c[b].area[m].coords,
                            function (a, b) {
                                var c = {
                            ...
                            h = k, f.preventDefault(), f.stopImmediatePropagation()
                }),
                    ...
                    function (c) {
                        ...
                        if (0 === c.button || f) {
                            var i = d.getPosition.apply(e, [b, c, f]),
                                j = d.getClientPosition.apply(e, [b, i]); g = a(this),
                                    h = j
                        }
                    });
            var i;
                ...
                function (f, g) {
                    ...
                    h.is("button") ? a(document).on("click", g.selector,
                        function () {
                            ...
                            function () {
                                var f = a(this),
                                    h = g.fn.apply(this), i = f.val();
                                "active" == h[1] ? (a(g.selector).not(this).attr("checked", !1), c[b].state.areaIndex = h[0],
        ...
        getRatio: function () {
            var b = a(".image-mapper-img", this); return {
        ...
        getPosition:
            function (b, c, e) {
                ...
                return e ? (i.naturalX = Math.round((c.originalEvent.targetTouches[0].pageX - g.left) * h.ratioX),
                    i.naturalY = Math.round((c.originalEvent.targetTouches[0].pageY - g.top) * h.ratioY)) : (i.naturalX = Math.round((c.clientX + (window.scrollX || window.pageXOffset) - g.left) * h.ratioX), i.naturalY = Math.round((c.clientY + (window.scrollY || window.pageYOffset) - g.top) * h.ratioY)),
                    i
            },
        ...
        renderSVG:
            function (b) {
                var e = this,
                    f = a(".image-mapper-svg", this); f.css("width", "100%"),
                        a(".image-mapper-shape", f).remove(),
                        a.each(c[b].area, function (g, h) {
                            var i, j = []; a.each(h.coords,
                                function (a, c) {
                                ...
                                h.el && (i = h.el), "poly" == h.shape ? (i || (i = a(document.createElementNS("http://www.w3.org/2000/svg", "polygon"))),
                                    i.attr("points",
                                        j.join(","))) : "circle" == h.shape ? j.length >= 4 && (i || (i = a(document.createElementNS("http://www.w3.org/2000/svg", "circle"))),
                                            i.attr("cx", j[0]).attr("cy", j[1]),
                                            i.attr("r",
                                                Math.sqrt(Math.pow(j[2] - j[0], 2) + Math.pow(j[3] - j[1], 2)))) : j.length >= 4 && (i || (i = a(document.createElementNS("http://www.w3.org/2000/svg",
                                                    "rect"))),
        ...
        renderPoints:
            function (b) {
                var e = this, f = a(".image-mapper-svg", this); a(".image-mapper-point", f).remove();
                var g = c[b].state.areaIndex,
                    h = c[b].area[g];
                a.each(h.coords,
                    function (h, i) {
                        var j = a(document.createElementNS("http://www.w3.org/2000/svg", "circle")),
                            k = d.getClientPosition.apply(e, [b, i]); j.attr("cx", k.clientX).attr("cy", k.clientY), j.attr("r", 5),
                                j.attr("class", "image-mapper-point"), j.attr("data-area-index", g),
                                j.attr("data-coord-index", h),
                                j.css(c[b].options.pointStyle), f.append(j)
                    })
            }, asHTML:
            function (b) {
                var d = a("<img />"),
    ...
    a.fn.imageMapper =
        function () {
            ...
            return "getData" == b ? d.getData(f) : "asHTML" == b ? d.asHTML(f) : (e.unshift(f),
                this.each(function () { "function" == typeof d[b] && d[b].apply(this, e) }))
        }
}
    ...
    function (a) {
        a(document).on("init", function () {
            function b(a) {
                for (var b = "", c = new Uint8Array(a),
                    d = c.byteLength,
                    e = 0; d > e; e++)b += String.fromCharCode(c[e]);
            ...
            a(document).on("click", "button.add-row",
                ...
                a(document).on("update", "#image-map", function () {
                    ...
                    var b = a(this).imageMapper("getData"),
                        c = a("#image-mapper-table"), d = imageMapperRow;
                    a("tbody", c).html(""),
                ...
                a("#image-map").imageMapper({
                    src: "",
                    event: {
                        init: function () { a("#image-map").trigger("update") },
                        update: function () { a("#image-map").trigger("update") },
                        removeArea: function () { a("#image-map").trigger("update") }
                    }
                }),
                ...
                a(document).on("click", "#code", "comment",
                    function () {
                        // alert("hola");
                        var b = a("#image-map").imageMapper("asHTML");
                        a("#modal-code-result").text(b).css()
                        console.log(b);
                    }),
            ...
            var c = {
                ...
                error: function () {
                    var b = a("#image-mapper-dialog"); a("#image-mapper-continue", b).attr("disabled", "disabled"),
                ...
                a(document).on("click", "#image-mapper-continue",
                    function () {
                        ...
                        a("#image-map").imageMapper("update", {
                            src: a("#image-mapper-url", b).val(), file: ""
                        }), a("#image-mapper-load").modal("hide")
                    }),
                a(document).on("change", "#image-mapper-file",
                    function () {
                        var c = this.files[0],
                            d = new FileReader; d.onloadend = function (d) {
                                ...
                                //tener en cuenta la informacion de los datos de la imagen
